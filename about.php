
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta name="generator" content="ORNELIO REIPH">
        <meta name="dcterms.created" content="ma, 10 dec 2018 9:45:23 GMT">
        <meta name="description" content="">
        <meta name="keywords" content="">
			
	<link rel="stylesheet" href="css/index.css" />
	<link rel="stylesheet" href="css/navigation.css" />
        <title>About</title>
        	  
    </head>

<body>
<br />
  <div class="menu">
	  <div class="menu1">
         <a href="index.php">Virtual Market</a>
         <a href="terms/terms.php">Voorwaarden</a>
         <a href="#">Veilig handelen</a>
	  </div>
	  
	  <div class="menu2">
	     <a href="#">Berichten</a>
	     <a href="#">Meldingen</a>
	     <a href="#">Inloggen</a> 
		 <a href="about.php">About</a>
	  <input type="button" value="Plaats advertentie"/>
	  </div>
	
 </div>
	
	<br />
	<br />
	<br />
	<br />
	<!-- =========================================================================
	================================Search Menu===================================
	========================================================================== -->
<div class="header">
   
 <div class="Searchmenu" align="center">
<input class="searchInput" type="text" name="searchInput" placeholder="Search" required>

<select class="searchSelect" name="searchSelect" align="top">
   <option selected value="1">Alle groepen</option>
   <option value="2">Antiek en Kunst</option>
   <option value="2">Adio, TV, Foto's</option>
   <option value="2">Auto's</option>
   <option value="3">auto-onderdelen</option>
   <option value="2">auto-diversen</option>
   <option value="4">Boeken</option>
   <option value="2">Caravans en Kamperen</option>
   <option value="5">Cd's en Dvd'S</option>
   <option value="2">Computers en Software</option>
   <option value="6">Contacten en Berichten</option>
   <option value="2"d>Diensten en Vakmensen</option>
   <option value="7">Dieren en Toebehoren</option>
   <option value="2">Doe-Het-Zelfen Verbouw</option>
   <option value="8">Fietsen en Brommers</option>
   <option value="2">Hobby en Vrije tijd</option>
   <option value="9">Huis en Inrichting</option>
   <option value="2">Antiek</option>
   <option value="10">Huizen en Kamers</option>
   <option value="2">Kinderen en Baby's</option>
   <option value="11">Kleding Dames</option>
   <option value="2">Kleding Heren</option>
   <option value="12">Motoren</option>
   <option value="13">Muziek en Instrumenten</option>
</select>

<input class="PosctcodeInput" type="text" name="searchInput" placeholder="Posctode" required>

<select name="Afstand" class="afstand">
  <option selected value="0">Alle Afstanden</option>
  <option value="1">3KM</option>
  <option value="2">5KM</option>
  <option value="3">10KM</option>
  <option value="4">15KM</option>
  <option value="5">25KM</option>
  <option value="6">50KM</option>
  <option value="7">75KM</option>
</select>

 <input type="button" value="ZOEK" class="btnZoek">
 </div>
 </div>

  <br />
  <br />
  <br />
  <br />
  <br />

  
<!-- =========================================================================
	================================About ===================================
	========================================================================== -->
<div class="body">
<h1>About</h1>
<p id="text1">
   <br />
   This ad website was created by Ornelio reiph. <br />
   You are free to add advertisements. <br />
   Create an account via the 'register menu' to add advertisements. <br />
   There is the possibility to place pictures or other 'files with your advertisement. <br />
   <br />
   You can contact the 'administrator' through the 'contact-menu'. <br /> <br />
   
   With kind regards, <br />
   Ornelio Reiph <br />
   Administrator. <br />
   </p>
   <img id="img1" src="images/nelio1.png" width="192" height="192" alt="" title="blog administartor" />
   
   
    <div class="footer">
  jgkjhjjkljlk
  </div>
</div>




  </body>
</html>